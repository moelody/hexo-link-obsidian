...
export const convertLinksAndSaveInSingleFile = async (mdFile: File) => {
    let fileText = await fs.readFile(mdFile.source)
...
const createLink = async (dest: LinkType, originalLink: string, altOrBlockRef: string, sourceFile: File) => {
    ...
    // 文件对象
    let file = await getFirstLinkpathDest(fileLink, readlinkSync(sourceFile.source))
...
function getAbsoluteLink(file: TFile) {
    ...
    fileLink = file.vault.adapter.basePath + '/' + file.path
